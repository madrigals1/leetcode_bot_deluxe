...
| `VIZAPI_URL` | Base URL of the VizAPI chart generation service |
...
| `/rating_cml` | Cumulative (weighted) rating leaderboard | |
| `/profile [username]` | View a user's LeetCode profile | |
| `/avatar [username]` | View a user's LeetCode avatar | |
| `/langstats [username]` | View language statistics | |
| `/submissions [username]` | View recent submissions table | |
...
```bash
npm run start        # Start in dev mode (watch)
